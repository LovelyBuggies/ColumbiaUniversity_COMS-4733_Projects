...
class PyBulletSim:
    ...
    def __init__(self, use_random_objects=False, object_shapes=None, gui=True):
        ...
        self._workspace1_bounds = np.array([[0.38, 0.62], [-0.22, 0.22], [0.00, 0.5]])  # 3x2 rows: x,y,z cols: min,max
        ...
        # load UR5 robot
        self.robot_body_id = p.loadURDF("assets/ur5/ur5.urdf", [0, 0, 0.4], p.getQuaternionFromEuler([0, 0, 0]))
        self._mount_body_id = p.loadURDF("assets/ur5/mount.urdf", [0, 0, 0.2], p.getQuaternionFromEuler([0, 0, 0]))
        ...
        self._tote1_position = (self._workspace1_bounds[:, 0] + self._workspace1_bounds[:, 1]) / 2
        self._tote1_position[2] = 0.01
        self._tote1_body_id = p.loadURDF(
            "assets/tote/toteA_large.urdf",
            self._tote1_position,
            p.getQuaternionFromEuler([0, 0, 0]),
            useFixedBase=True,
        )

        self._tote2_position = (self._workspace2_bounds[:, 0] + self._workspace2_bounds[:, 1]) / 2
        ...
        self._object_shape_ids = [i % len(self._object_shapes) for i in range(self._num_objects)]
        ...
        # Add obstacles
        self.obstacles = [
            p.loadURDF("assets/obstacles/block.urdf", basePosition=[0, 0.65, 0.9], useFixedBase=True,),
            p.loadURDF("assets/obstacles/block.urdf", basePosition=[0, 0.65, 0.3], useFixedBase=True,),
            p.loadURDF("assets/obstacles/block.urdf", basePosition=[0, -0.65, 0.9], useFixedBase=True,),
            p.loadURDF("assets/obstacles/block.urdf", basePosition=[0, -0.65, 0.3], useFixedBase=True,),
            p.loadURDF("assets/obstacles/block.urdf", basePosition=[0, 0, 1.5], useFixedBase=True,),
        ]
        self.obstacles.extend([self._plane_id, self._tote1_body_id, self._tote2_body_id])
    ...
    def load_gripper(self):
        ...
        p.resetBasePositionAndOrientation(
            self._gripper_body_id, [0.5, 0.1, 0.2], p.getQuaternionFromEuler([np.pi, 0, 0]),
        )
    ...
    def move_joints(self, target_joint_state, speed=0.03):
        ...
        while True:
            ...
            current_joint_state = [p.getJointState(self.robot_body_id, i)[0] for i in self._robot_joint_indices]
            if all(
                [
            ...
            if time.time() - timeout_t0 > 5:
                print("Timeout: robot is taking longer than 5s to reach the target joint state. Skipping...")
                p.setJointMotorControlArray(
                    self.robot_body_id,
                    self._robot_joint_indices,
                    p.POSITION_CONTROL,
    ...
    def close_gripper(self):
        p.setJointMotorControl2(self._gripper_body_id, 1, p.VELOCITY_CONTROL, targetVelocity=5, force=10000)
    ...
    def execute_place(self, place_angle=90.0):
        gripper_orientation = p.getQuaternionFromEuler(
            [np.pi, 0, ((place_angle + 180.0) % 360.0 - 180.0) * np.pi / 180.0]
        )
        ...
        for i in range(int(num_steps)):
            ...
            if self._gripper_body_id is not None:
                ...
                gripper_joint_positions = np.array(
                    [p.getJointState(self._gripper_body_id, i)[0] for i in range(p.getNumJoints(self._gripper_body_id))]
                )
                p.setJointMotorControlArray(
                    self._gripper_body_id,
                    [6, 3, 8, 5, 10],
    ...
    def reset_objects(self):
        for object_body_id in self._objects_body_ids:
            random_position = (
                np.random.random_sample((3)) * (self._workspace1_bounds[:, 1] - (self._workspace1_bounds[:, 0] + 0.1))
                + self._workspace1_bounds[:, 0]
                + 0.1
            )
    ...
    def check_collision(self, q):
        ...
        for obstacle_id in self.obstacles:
            closest_points = p.getClosestPoints(self.robot_body_id, obstacle_id, 0.18)
...
class SphereMarker:
    def __init__(
        self, position, radius=0.05, rgba_color=(1, 0, 0, 0.8), text=None, orientation=None, p_id=0,
    ):
        self.p_id = p_id
        position = np.array(position)
        vs_id = p.createVisualShape(p.GEOM_SPHERE, radius=radius, rgbaColor=rgba_color, physicsClientId=self.p_id,)
        ...
        if text is not None:
            self.debug_item_ids.append(p.addUserDebugText(text, position + radius))
        ...
        if orientation is not None:
            ...
            self.debug_item_ids.append(p.addUserDebugLine(position, x_end, lineColorRGB=(1, 0, 0)))
            # y axis
            y_end = np.array([[0, axis_size, 0]]).transpose()
            y_end = np.matmul(rotation_mat, y_end)
            y_end = position + y_end[:, 0]
            self.debug_item_ids.append(p.addUserDebugLine(position, y_end, lineColorRGB=(0, 1, 0)))
            # z axis
            z_end = np.array([[0, 0, axis_size]]).transpose()
            z_end = np.matmul(rotation_mat, z_end)
            z_end = position + z_end[:, 0]
            self.debug_item_ids.append(p.addUserDebugLine(position, z_end, lineColorRGB=(0, 0, 1)))
